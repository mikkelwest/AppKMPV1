...
class SecApi(private val client: HttpClient, private val baseUrl: String) {
    ...
    suspend fun login(email: String, password: String): TokenResponse? {
        ...
        return try {
            ...
            if (!tok.accessToken.isNullOrBlank() && !tok.refreshToken.isNullOrBlank()) {
                tokenStore.setTokens(tok.accessToken, tok.refreshToken)
            }
            tok
        } catch (e: Exception) {
    ...
    suspend fun refresh(email: String, refreshToken: String): TokenResponse? {
        ...
        val resp = client.post("$baseUrl/admin/refreshtoken") { setBody(req) }
        val raw = resp.bodyAsText()
        return try {
            val tok = json.decodeFromString<TokenResponse>(raw)
            if (!tok.accessToken.isNullOrBlank() && !tok.refreshToken.isNullOrBlank()) {
